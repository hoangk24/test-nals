...
export default function Profile() {
  ...
  const obSubmit = (value: any) => {
    if (!file) {
      toast.error('Hãy thêm ảnh đại diện mới!')
      return
    }
    const data = {
      'me[name]': value.name,
    ...
    updateProfile(formData, () => setShow(false))
  }
        ...
        <form onSubmit={handleSubmit(obSubmit)}>
          ...
          <Modal.Body>
            ...
            <Input
              ...
              label='Email'
              control={control}
              rules={{ validate: (values: string) => RULE.WrongEmailFormat(values) }}
            />
            <Input
              name='password'
              type='password'
              label='Mật khẩu'
              rules={{ validate: (values: string) => RULE.atLeast8Characters(values, 'Mật khẩu') }}
              control={control}
            />
          </Modal.Body>
          <Modal.Footer>
