...
export const RULE = {
  ...
  WrongEmailFormat(value: string) {
    if (!value) return replaceMessage(MESSAGE.Required, 'Email')
    if(!/^[\w-\.]+@([\w-]+\.)+[\w-]{2,4}$/.test(value)) return replaceMessage(MESSAGE.WrongEmailFormat`, name)
  },
}
